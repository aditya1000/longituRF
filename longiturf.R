...
MERFranger <- function(X,Y,id,Z,iter,mtry, ntree, time, sto, delta = 0.001){
  ...
  if (class(sto)=="character"){
    if (sto=="fbm"){
      ...
      return(list(forest=forest,random_effects=btilde,var_random_effects=Btilde,sigma=sigmahat, id_btilde=unique(id),omega=omega, sigma_sto =sigma2, time = time, sto= sto, Hurst =h, id=id, Vraisemblance=Vrai))
    }
    ...
    if (sto=="exp"){
      ...
      return(list(forest=forest,random_effects=btilde,var_random_effects=Btilde,sigma=sigmahat, id_btilde=unique(id), omega=omega, sigma_sto =sigma2, time = time, sto= sto, alpha=alpha, id=id, Vraisemblance=Vrai))
    }
...
predict.merfRanger <- function(merf_sto, X,Z,id,time){
  n <- length(unique(id))
  id_btilde <- merf_sto$id_btilde
  f <- predict(merf_sto$forest,X)$predictions
  Time <- merf_sto$time
  id_btilde <- merf_sto$id_btilde
